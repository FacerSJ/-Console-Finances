...
// Creating loop to calculate the total of Profit/Losses
...
// add variable for the total change in Profit/Losses

var changes = [];

// create a loop to subtract the amount with the previous months amount.
...
// get the average change by deviding sum with changes length.

var averageChange = sum / (changes.length);

console.log("Average change: $" + averageChange.toFixed(2));


//  4.  find the greatest increase in Profit / Loss over the entire period.
...
for (var i = 1; i < finances.length; i++) { 
    ...
    //  If statemant to show if the change in profit is greater than the greatest increase amount, update the greatest increase variables. 
    if (changeInProfit > greatestIncreaseAmount) {
        greatestIncreaseDate = finances[i][0];
        greatestIncreaseAmount = changeInProfit;

    // else if the change in profits is less than the current greatest decrease in profits, update the greatest decrease variables
